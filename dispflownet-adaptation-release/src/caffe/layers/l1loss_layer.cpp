...
void L1LossLayer<Dtype>::LayerSetUp(const vector<Blob<Dtype>*>& bottom,
  ...
  if(bottom.size() == 1) {
    ...
    if(bottom.size()==3){
      ...
      //Element wise dot product layer
      dot_top_vec_.clear();
      dot_top_vec_.push_back(&dot_);
      dot_bottom_vec_.clear();
      dot_bottom_vec_.push_back(diff_top_vec_[0]);
      dot_bottom_vec_.push_back(bottom[2]);
      LayerParameter dot_param;
      dot_param.mutable_eltwise_param()->set_operation(EltwiseParameter_EltwiseOp_PROD);
      dot_layer_.reset(new EltwiseLayer<Dtype>(dot_param));
      dot_layer_->SetUp(dot_bottom_vec_,dot_top_vec_);
    } else {
