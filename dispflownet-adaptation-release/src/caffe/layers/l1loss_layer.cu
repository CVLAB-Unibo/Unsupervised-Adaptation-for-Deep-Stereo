      ...
      const vector<Blob<Dtype>*>& top)
{
  ...
  Blob<Dtype> *diffptr = dot_top_vec_[0];
  ...
  if(bottom.size() > 1) {
    ...
    if(bottom.size()>2){
      dot_layer_->Forward(dot_bottom_vec_, dot_top_vec_);
    }
  }
  ...
  if (this->layer_param_.l1_loss_param().l2_per_location()) {
    ...
    square_layer_->Forward(dot_top_vec_, square_top_vec_);
...
{  
  ...
  if (prop_down) {
    ...
    if (this->layer_param_.l1_loss_param().l2_per_location()) {
      ...
      square_layer_->Backward(square_top_vec_, prop_down, dot_top_vec_);
    ...
    if(bottom.size() == 2) {
        diff_layer_->Backward(dot_top_vec_, propagate_down, bottom);
    //NUOVO
    } else if(bottom.size() == 3){
      dot_layer_->Backward(dot_top_vec_, propagate_down, dot_bottom_vec_);
