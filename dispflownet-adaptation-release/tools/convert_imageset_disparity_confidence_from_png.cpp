...
public:
    ...
    ImagePair(std::string line)
    {
        clrptr();
        stringstream s(line);
        s >> img1_filename >> img2_filename >> disp_filename >> conf_filename;
    }
    ~ImagePair()
    {
        if(data) delete data;
    ...
    bool has_conf() const { return conf_filename!=""; }
    ...
    void read_data()
    {
        ...
        if(has_disp())
        {
            //cv_disp = cv::imread(disp_filename, CV_LOAD_IMAGE_GRAYSCALE);
            cv_disp = cv::imread(disp_filename, CV_LOAD_IMAGE_UNCHANGED);
            if (!cv_disp.data) LOG(FATAL) << "Could not open or find file " << disp_filename;
        ...
        if(has_conf())
        {
              cv_conf = cv::imread(conf_filename, CV_LOAD_IMAGE_GRAYSCALE);
              if (!cv_conf.data) LOG(FATAL) << "Could not open or find file " << conf_filename;
              //cv::Mat cv_conf_tmp = cv::imread(conf_filename, CV_LOAD_IMAGE_UNCHANGED);
              //cv_conf_tmp.convertTo(cv_conf, CV_8U, 1.0/256.0);
        }
        ...
        data_size=  3*width*height              // Image 1
                   ...
                   +2*(width*height)             //depth - 16 bit
                   +width*height;                //confidence
//                   +2*width*height;                //confidence - 16 bit                                  
        ...
        for (int y = 0; y < height; ++y)
            for (int x = 0; x < width; ++x) {
                ...
                if(has_disp())
                {
                    if (cv_disp.depth() == CV_16U){
                        if (isnan(cv_disp.at<ushort>(y, x)) || y >= cv_disp.rows || x >= cv_disp.cols || cv_disp.at<ushort>(y, x) == 0)
                        {
                            value = std::numeric_limits<short>::max();
                        }
                        else
                            value = -1.0 * (cv_disp.at<ushort>(y, x) / 256.0); // = flo_data[j]*32.768/1024;
                    } else {
                        if (isnan(cv_disp.at<char>(y, x)) || y >= cv_disp.rows || x >= cv_disp.cols || cv_disp.at<char>(y, x) == 0)
                        {
                            value = std::numeric_limits<short>::max();
                        }
                        else{
                            value = (short)(-1.0 * cv_disp.at<char>(y, x)); // = flo_data[j]*32.768/1024;
                        }
                    }
                }
        ...
        //read confidence
         for (int y = 0; y < height; ++y)
		        ...
		        unsigned char value=0;
                    if( y >= cv_conf.rows || x >= cv_conf.cols)
                        value = 0;
                    else
                        value=cv_conf.at<unsigned char>(y, x);
		    //unsigned short value=cv_conf.at<unsigned short>(y, x);
                    *(ptr++)=*((unsigned char*)&value);
                    //*(ptr++)=*((unsigned char*)&value);
                    //*(ptr++)=*((unsigned char*)&value+1);
                }
        assert(ptr==data+9*width*height);
    }
